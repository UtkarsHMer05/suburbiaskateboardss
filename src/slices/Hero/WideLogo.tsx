"use client"; // Tells Next.js this component needs to run in the browser (client-side), not on the server

import { SVGProps } from "react"; // Imports types for SVG properties so TypeScript knows what attributes are valid
import clsx from "clsx"; // Imports a utility that helps combine CSS class names conditionally

import { useIsSafari } from "@/lib/useIsSafari"; // Imports our custom hook that checks if the user is on Safari browser

/**
 * WideLogo Component:
 * ------------------
 * This component displays the wide/horizontal version of the Suburbia Skateboards logo.
 * It's designed for larger screens where a horizontal logo fits better.
 *
 * How it works:
 * 1. Takes in any standard SVG properties (like className for styling)
 * 2. Checks if the browser is Safari (for animation compatibility)
 * 3. Renders the logo SVG with appropriate classes and attributes
 *
 * @param className - CSS classes to apply to the SVG
 * @param props - Any other standard SVG attributes to pass through
 */
export function WideLogo({ className, ...props }: SVGProps<SVGSVGElement>) {
  /**
   * Browser Detection:
   * ----------------
   * Check if the user is browsing with Safari.
   * This matters because some animations don't work well in Safari.
   *
   * useIsSafari(true) - The 'true' means temporarily assume it's Safari until we can check,
   * which prevents flickering if loading on Safari.
   */
  ...
  return (
     ...
     * SVG Element:
     * -----------
     * Creates the container for our logo with these attributes:
     *
     * • xmlns - Required XML namespace for SVG standard
     * • fill="none" - Makes the SVG background transparent
     * • viewBox - Defines the coordinate system (width 1440, height 553)
     * • className - Combines passed class names with conditional animation:
     *   - Uses clsx to merge the classes
     *   - Only applies "animate-squiggle" animation if NOT using Safari
     *   - This animation makes the logo wiggle slightly for a hand-drawn feel
     * • {...props} - Includes any other properties passed to the component
     */
    <svg
      ...
      {...props}
    >
      /** * Path Element: * ------------ * This defines the actual shape of the
      logo: * * • fill="currentColor" - Takes its color from the parent
      element's text color * This means we can change the logo color just by
      setting the text color of its container * * • d="M211 222c-5 13..." - The
      path data that creates the shape * This is a complex set of drawing
      commands that trace the outline of the * "Suburbia Skateboards" text in a
      stylized, hand-drawn looking font. * * The path is written in a compressed
      format where: * - M = Move to a point * - c = Curve relative to current
      position * - m = Move relative to current position * * Each number
      represents x,y coordinates or control points for the curves. */
      <path
        fill="currentColor"

...
import { SVGProps } from "react"; // Imports types for SVG element properties from React
import clsx from "clsx"; // Imports a utility that helps combine CSS class names conditionally

import { useIsSafari } from "@/lib/useIsSafari"; // Imports our custom hook that detects Safari browsers

/**
 * TallLogo Component:
 * ------------------
 * This component renders a tall/portrait version of the Suburbia Skateboards logo.
 * It's optimized for mobile devices or narrow screens where a vertical logo works better.
 *
 * The component handles:
 * • Rendering the SVG logo with the correct dimensions
 * • Applying browser-specific styling (disabling animations on Safari)
 * • Passing through any additional SVG properties
 *
 * @param className - CSS classes to apply to the SVG element
 * @param props - Any other SVG attributes to pass through to the element
 */
export function TallLogo({ className, ...props }: SVGProps<SVGSVGElement>) {
   ...
   * We do this because some animations work differently in Safari.
   * The 'true' parameter means we assume it's Safari until we can check.
   */
  ...
  return (
     ...
     * SVG Element:
     * -----------
     * This creates the vector graphic container with these properties:
     *
     * • xmlns - Required XML namespace for SVG standard
     * • fill="none" - Default fill is transparent
     * • viewBox - Defines the coordinate system (0,0 to 366,420)
     * • className - Combines passed classes with conditional animation
     *   - Uses clsx to merge the classNames
     *   - Only applies animate-squiggle if NOT using Safari
     * • {...props} - Spreads any other properties passed to the component
     */
    <svg
    ...
    >
       ...
       * Path Element:
       * ------------
       * This defines the actual shape of the logo:
       *
       * • fill="currentColor" - Uses the current text color of its parent
       *   This allows the logo color to be controlled by CSS from outside
       *
       * • d="M95 109c-3 6..." - The path data that defines all the curves and lines
       *   This is a compressed set of drawing commands that create the detailed
       *   shape of the Suburbia Skateboards logo in portrait orientation
       */}
      <path
        fill="currentColor"
